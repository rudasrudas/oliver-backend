...
module.exports = function (app) {
    ...
    app.get("/household/:hhid", auth.verify, async (req, res) => {
        try {
            ...
            if (household == null)
                return res.status(400).send("Household ID is invalid");

            // check if logged in user is a household member
            const isHouseholdUser = await HouseholdUser.findOne({
    ...
    app.post("/household", auth.verify, async (req, res) => {
        try {
            ...
            if (!(name && address && currency)) {
                ...
                if (await HouseholdService.underFour(user)) {
                    //problem
        ...
        try {
            ...
            if (!caller)
                return res
                    .status(403)
                    .send(
                        "Logged in user does not have access to this function"
                    );
    ...
    app.post("/join/household/:hhid", auth.verify, async (req, res) => {
        ...
        try {
            ...
            if (!household)
                return res
                    .status(403)
                    .send("Household access is invitation only");
            ...
            if (key !== household.join_key)
                return res
                    .status(400)
                    .send("Key is incorrect for the household");
